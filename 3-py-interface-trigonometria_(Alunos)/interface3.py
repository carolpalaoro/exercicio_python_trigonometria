...
import math # Importa a biblioteca math para realizar operações matemátcas
from PIL import Image, ImageTk # Importa as classes Image e ImageTk da biblioteca para manipulação de imagens
...
def calcular():
    ...
    try:
        ...
        # Em caso de erro (por exemplo, entrada invalida), exibe "Erro" nas labels
...
def limpar():
    """
    Limpar a entrada do usuário e reseta as labels dos resultados.
    """
...
try:
    icone_path = resource_path("3-py-interface-trigonometria_(Alunos)\seno.png") # Obtém o caminho da imagem do ícone
...
# Imagem seno2.png
try:
    imagem_path = resource_path("3-py-interface-trigonometria_(Alunos)\seno2.png") # Obtém o caminho da imagem principal
...
botao_limpar = tk.Button(frame_botoes, text="Limpar", command=limpar, font=('Arial', 12),
                         bg="#d9d9d9", relief='flat', bd=0, highlightthickness=0) # Botão para limpar a entrada e os rresultados
botao_limpar.pack(side=tk.RIGHT, padx=10) # Posiciona o botão à direita com um espaçamento horizontal

# Resultados
frame_resultados = tk.Frame(janela, bg="#f0f0f0") # Criar um frame para organizar os resultados
...
resultado_tangente.grid(row=2, column=1, padx=10, pady=5, sticky='w') # Label que exibe o resultado da tangente à esquerda
